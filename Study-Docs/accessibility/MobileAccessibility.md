...
2. aria-label
3. aria-labelledby > 시각적 정보를 대체 텍스트로 연결해 제공
4. 국가 언어별에 맞는 대체텍스트 제공
5. 업데이트 정보 누락 예방 (광고 영역에 AD표기 뿐만 아니라, **광고 배너**라 추가)
6. 유형 정보 반복 제공 방지
...
> 화면에 표시되는 모든 사용자 인터페이스 컴포넌트와 텍스트는 전경색과 배경색이 구분될 수 있도록 제공되어야 한다.
   1. 화면에 표시되는 모든 사용자 인터페이스 컴포넌트와 텍스트는 전경색과 배경색이 구분될 수 있도록 모든 정보의 최소 대비는 4.5:1 이상이어야 함
      단, 운영체제의 **화면 확대 기능을 이용할 수 있는 경우, 명도 대비를 3:1 이상**으로 제공하면 됨
      * 명도 대비를 높게 제공하기 어려운 경우, 운영 체제에서 제공하는 명도 대비를 활용할 수 있도록 제공. 혹은 애플리케이션 내에서 명도 대비를 높일 수 있는 기능을 제공해야 함.
      * 사진, 동영상, 로고, 장식용 이미지를 제외한 모든 요소는 3:1 이상 되도록 준수해야 함
...
### 알림 기능
> 알림 정보는 화면 표시, 소리, 진동 등 두 가지 이상의 다양한 방법으로 제공되어야 한다.
   1. 중요한 알림 정보는 시각, 청각, 촉각 등 다양한 감각으로 인식될 수 있어야 함
   2. 알림 정보는 사용자가 자신에게 적합한 방법을 선택할 수 있도록 제공하는 것이 바람직함
   3. 일시적으로 나타났다 사라지는 토스트 팝업 등의 정보 또한 스크린 리더 음성 정보로 알려주어야 함 (aria-live="assertive")
...
> 의미나 기능을 갖는 모든 사용자 인터페이스 컴포넌트에는 초점(focus)이 적용되고, 초점은 논리적인 순서로 이동되어야 한다. 
   ...
   3. 표시되는 초점의 영역은 그룹별로 이동해야 하며, 콘텐츠의 위치와 크기가 맞도록 제공해야 함
...
### 입력 도움
> 입력 요소(입력 상자, 선택 상자, 라디오 버튼, 스위치)의 용도와 목정 정보 제공. 입력 서식 이용 시, 입력 오류를 방지하거나 정정할 수 있는 방법을 제공해야 한다.
   1. 입력 서식에는 용도와 목적을 알 수 있는 대체 정보를 제공해야 함
      > 제공하기 어려울 경우 ```title``` 혹은 ```aria-label```속성을 제공, <br/>
      > 또는 ```aria-labelledby```를 통해 시각적 제목 요소를 연결할 수 있음
...
### 사용자 인터페이스의 일관성
> 저시력, 지적 장애, 고령자를 위한 지침으로, 사용자 인터페이스 컴포넌트들은 일관성 있게 배치되어야 한다.
...
### 깜박거림의 사용 제한
> 광과민성 발작 장애가 있는 사용자에게 발작을 유발할 수 있으니, 깜빡이거나 번쩍이는 콘텐츠를 제공하지 않아야 한다.
...
> 자동으로 재생되는 배경음을 사용하지 않아야 한다.
   ...
   2. 배경음을 사용할 경우, 사용자가 손쉽게 멈춤, 일시 정지, 음량조절(음소거 상태 제공) 등과 같이 이를 제어할 수 있는 수단을 사용자에게 제공해야 함
