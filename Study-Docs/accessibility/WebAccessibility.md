...
1) 시간 연장이 불가능한 콘텐츠를 사용 시, 충분한 시간을 제공하거나 최소 20초 전에 **연장 수단 제공을 안내**해야 함
2) 페이지 자동 전환 시 시간 연장을 위한 멈춤 버튼 빛 재생 버튼 수단을 제공해야 함
...
#### 깜빡임과 반짝임 사용 제한
> 광과민성 증후군 예방을 위하여 초당 3~50회 주기로 깜빡이거나 번쩍이는 콘텐츠를 제공하지 않아야 함
...
※ 데이터가 아닌 콘텐츠는 table로 마크업하지 않길 권장
...
3) 레이아웃 표
   > 디자인을 위한 표는 표로 인식하지 않도록 제목과 요약 내용을 제공하지 않음. 만약 table로 제공한 경우, caption과 summary를 제공하지 않아야 함
...
1. 입력 오류 시 입력 내용이 모두 사라지는 경우
   > 입력 완료 후나 제출 시 오류가 발생하면 내용이 사라지지 않고 유지되도록 제공
2. 오류 내용 제공
   > 오류의 원인을 알 수 있도록 제공
3. 오류 발생 시점으로 초점 이동
   > 오류가 발생한 서식으로 초점 이동
